...
a <- list(text = "Points Scored by Dublin in Top Venues",xref = "paper",yref = "paper",
...
plot <- ggplot(gaa_croke, aes(x= Winner_Score, y = Winner, color = Winner)) + geom_jitter(width = 0.5, size = 2.5) + labs(y = NULL, x = "Score") + scale_colour_brewer(palette = "Set2") +theme(legend.position="none")
croke_plot <- ggplotly(plot, tooltip = c("x","y","colour")) %>%
...
gaa_dublin <- gaa_dublin[gaa_dublin$Winner_Score !=0,]
gaa_dublin <- gaa_dublin %>% filter(Winner != "OTHER TEAM")


plot2 <- ggplot(gaa_dublin, aes(x= Winner_Score, y = Venue, color = Venue)) + geom_jitter(width = 0.5, size = 2.5) + labs(y = NULL, x = "Score") + scale_colour_brewer(palette = "Set3") + theme(legend.position="none")
dub_plot <- ggplotly(plot2, tooltip = c("x","y","colour")) %>%
